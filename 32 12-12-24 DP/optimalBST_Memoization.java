...
import java.util.*;
public class optimalBST_Memoization {
    ...
    public static int helper(int[] freq, int i, int j, int[][] dp){
        if(j<i){
            return 0;
        }
        if(i==j){
        ...
        for(int r = i;r<=j;r++){
            min = Math.min(min, helper(freq, i,r-1,dp) + helper(freq, r+1, j, dp));
        }
        return dp[i][j] = min + freqSum;
